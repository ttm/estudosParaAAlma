...
print(f'ndim: {ndim}, ang: {ang}')

# normalization:
margin = 0.1
ix1 = (WIDTH * (1 - 2 * margin)) * (ix1 - ix1.min()) / (ix1.max() - ix1.min())
iy1 = (HEIGHT * (1 - 2 * margin)) * (iy1 - iy1.min()) / (iy1.max() - iy1.min()) + margin * WIDTH

breakAll = False
k16=0
for k15 in range(k16, k16 + 2 ** 16 + 1, 2 ** 16):
    ...
    if ndim == 16: breakAll = True  # goto 100
print('reached end, dim = ', ndim)
comp = ['', '_'][ang == 45]
surface.write_to_png(f'earl-{ndim}d{comp}.png')  # Output to PNG
surface.finish()
